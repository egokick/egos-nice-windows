...
function Configure-PrivateRustDeskController {
    ...
    $configRoots = @((Join-Path $ProfilePath 'AppData\Roaming\RustDesk\config'),(Join-Path $env:APPDATA 'RustDesk\config'),(Join-Path $env:WINDIR 'ServiceProfiles\LocalService\AppData\Roaming\RustDesk\config')) | Select-Object -Unique
