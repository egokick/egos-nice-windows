...
internal sealed class AdminPanelForm : Form
{
    ...
    private bool _centeredOnScreen;
    ...
    public AdminPanelForm()
    {
        ...
        _appGrid = new FlowLayoutPanel
        {
            ...
            WrapContents = true,
            Padding = new Padding(18, 18, 18, 26),
            TabStop = false
        };
    ...
    private void CenterOnCurrentScreen()
    {
        ...
        _centeredOnScreen = true;
        var workingArea = Screen.FromPoint(Cursor.Position).WorkingArea;
        var width = Math.Min(1180, Math.Max(480, workingArea.Width - 56));
        var height = Math.Min(780, Math.Max(420, workingArea.Height - 56));
    ...
    private void UpdateWindowIcon()
    {
        var replacement = _palette.IsDark
            ? TrayIconFactory.CreateMoonIcon()
            : TrayIconFactory.CreateSunIcon();
...
internal sealed class AdminAppCard : UserControl
{
    ...
    private readonly RoundedActionButton _launchButton;
    ...
    public AdminAppCard(AdminAppDefinition definition, ToolTip toolTip)
    {
        ...
        {
            Size = new Size(76, 76),
            Image = AdminAppLogoFactory.Create(definition, 160),
        ...
        _launchButton = new RoundedActionButton
        {
            Height = 46,
            Font = new Font("Segoe UI", 10f, FontStyle.Bold, GraphicsUnit.Point),
            Text = "▶  Launch",
            AccessibleName = $"Launch {definition.DisplayName}",
            AccessibleDescription = $"Start {definition.DisplayName} using its project launcher."
        };
        _launchButton.Click += (_, _) => LaunchRequested?.Invoke(this, EventArgs.Empty);
        ...
        _startupToggle = new CheckBox
        {
            ...
            Text = "Start with Windows  •  OFF",
            TextAlign = ContentAlignment.MiddleCenter,
            AccessibleRole = AccessibleRole.CheckButton,
            AccessibleName = $"Start {definition.DisplayName} with Windows"
        };
        _startupToggle.FlatAppearance.BorderSize = 0;
        ...
        _toolTip.SetToolTip(_launchButton, $"Launch {definition.DisplayName}");
        _toolTip.SetToolTip(_startupToggle, $"Toggle Windows startup for {definition.DisplayName}");

        AttachDragSurface(this);
        AttachDragSurface(_logo);
        AttachDragSurface(_title);
        AttachDragSurface(_description);
    ...
    public void ApplyPalette(AdminPalette palette)
    {
        ...
        _description.BackColor = palette.Card;
        ...
        _launchButton.ApplyPalette(palette.Accent, palette.AccentHover, palette.AccentText);
    ...
    public void SetLaunchAvailability(bool available, string reason)
    {
        ...
        _launchButton.Enabled = available && !_launchBusy;
        _toolTip.SetToolTip(
            _launchButton,
    ...
    {
        _launchBusy = busy;
        _launchButton.Enabled = _launchAvailable && !busy;
        _launchButton.Text = busy ? busyText ?? "Launching…" : "▶  Launch";
    }

    public void SetLaunchResult(bool success, string? detail = null)
    {
        _launchButton.Text = success ? "✓  Requested" : "Could not start";
        _toolTip.SetToolTip(
            _launchButton,
    ...
    public void SetAutoStartState(bool? enabled, string errorMessage)
    {
        ...
        _startupToggle.Text = enabled switch
        {
            true => "Starts with Windows  •  ON",
            false => "Start with Windows  •  OFF",
            null => "Startup unavailable"
        };
        ...
        _toolTip.SetToolTip(
            ...
            string.IsNullOrWhiteSpace(errorMessage)
                ? $"Toggle Windows startup for {Definition.DisplayName}"
                : errorMessage);
        ApplyStartupTogglePalette();
    }
    ...
    public void SetAutoStartBusy(bool busy, bool targetState)
    {
        ...
        _startupToggle.Text = busy
            ? targetState ? "Enabling…" : "Disabling…"
            : targetState ? "Starts with Windows  •  ON" : "Start with Windows  •  OFF";
    }

    public void SetDragging(bool dragging)
    {
        _isDragging = dragging;
        Cursor = dragging ? Cursors.SizeAll : Cursors.Default;
        Invalidate();
    }
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        using var border = new Pen(_isDragging ? _palette.Accent : _palette.CardBorder, _isDragging ? 2.4f : 1.2f);
    ...
    private void ApplyRoundedRegion()
    {
        ...
        {
            AdminDrawing.SetRoundedRegion(_startupToggle, 9);
        }
    }
    ...
    private void LayoutCardContent()
    {
        ...
        _logo.Location = new Point((ClientSize.Width - _logo.Width) / 2, 18);
        _dragGrip.Location = new Point(ClientSize.Width - _dragGrip.Width - 11, 10);
        _title.Bounds = new Rectangle(16, 101, Math.Max(100, ClientSize.Width - 32), _title.Height);
        _description.Bounds = new Rectangle(18, 130, Math.Max(100, ClientSize.Width - 36), _description.Height);
        _launchButton.Bounds = new Rectangle(30, 184, Math.Max(100, ClientSize.Width - 60), _launchButton.Height);
        _startupToggle.Bounds = new Rectangle(30, 238, Math.Max(100, ClientSize.Width - 60), _startupToggle.Height);
    }
...
internal sealed class RoundedActionButton : Button
{
    ...
    public void ApplyPalette(Color background, Color hover, Color foreground)
    {
    ...
    {
        base.OnResize(e);
        AdminDrawing.SetRoundedRegion(this, 11);
    }
}
    ...
    bool IsDark)
{
    public static AdminPalette ForCurrentSystemTheme()
    {
        ...
        var lightMode = ThemeService.IsLightModeEnabled();
        return lightMode
            ? new AdminPalette(
