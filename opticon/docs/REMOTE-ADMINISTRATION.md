...
6. The command center requests a bounded duration rather than imposing its wall clock on the target. After infrastructure preparation, the Agent records the target-relative creation and expiry times atomically with revocation tombstones and a termination generation. The command center validates that returned interval without comparing clocks between computers. Every five seconds, the Guardian rewrites authorized keys with source-address and UTC expiry restrictions; revocation or expiry restarts its kill-on-close job so an already-authenticated shell closes even when another lease remains. Still-valid clients may reconnect. The maximum lease is eight hours and the normal UI lease is one hour.
