...
var tests = new (string Name, Action Body)[]
{
    ...
    ("Command Center release deployment preflights active invitations before publishing", TestReleaseDeploymentSurface),
...
{
    var disabledPolicy = ClientInstallValidationPolicy.Normalize(new ClientInstallValidationPolicy
    {
        DisableAll = true,
        DisabledSteps = Enum.GetNames<ClientInstallValidationStep>()
    });
    Assert(Enum.GetValues<ClientInstallValidationStep>().All(step => !disabledPolicy.IsEnabled(step)),
        "disable-all client installation policy leaves one or more validation steps enabled");

    Assert(RemoteAdministrationProtocol.IsCanonicalPrivateCoordinatorUrl("http://100.64.0.1:45830")
           && RemoteAdministrationProtocol.IsCanonicalPrivateCoordinatorUrl("http://100.127.255.254:45830/"),
        "Setup rejects the canonical private Tailscale coordinator URL generated by the command center");
    string[] invalidCoordinatorUrls =
    [
        "https://100.64.0.1:45830",
        "http://100.63.255.255:45830",
        "http://100.128.0.1:45830",
        "http://192.168.1.10:45830",
        "http://100.64.0.1:45831",
        "http://user@100.64.0.1:45830",
        "http://100.64.0.1:45830/api",
        "http://100.64.0.1:45830/?query=1",
        "http://100.64.0.1:45830/#fragment"
    ];
    Assert(invalidCoordinatorUrls.All(url =>
            !RemoteAdministrationProtocol.IsCanonicalPrivateCoordinatorUrl(url)),
        "private coordinator validation accepts an endpoint outside the fixed Tailscale origin");
    var setupPreflight = ReadSource("src", "Taildesk.Setup", "SetupPreflight.cs");
    Assert(setupPreflight.Contains(
            "RemoteAdministrationProtocol.IsCanonicalPrivateCoordinatorUrl(invite.CoordinatorUrl)",
            StringComparison.Ordinal),
        "Setup preflight does not use the shared private coordinator validator");
    var setupWindow = ReadSource("src", "Taildesk.Setup", "MainWindow.xaml.cs");
    var replacementGate = setupWindow.IndexOf(
        "if (validation.IsEnabled(ClientInstallValidationStep.SetupPreflight))",
        StringComparison.Ordinal);
    var replacementDiscovery = setupWindow.IndexOf(
        "var replacementPreflight = await SetupPreflight.DiscoverElevatedAsync(",
        StringComparison.Ordinal);
    Assert(replacementGate >= 0 && replacementDiscovery > replacementGate,
        "the elevated replacement handoff runs Setup preflight outside the published validation policy");

    var sourceBootstrap = ReadSource("src", "Taildesk.Setup", "SourceBootstrapInstaller.cs");
    Assert(sourceBootstrap.Contains("validateTransport: validateDownload", StringComparison.Ordinal)
           && sourceBootstrap.Contains("validateDownload ? 256L * 1024 * 1024 : long.MaxValue", StringComparison.Ordinal)
           && sourceBootstrap.Contains("validation.IsEnabled(ClientInstallValidationStep.SourceArchiveAuthenticity)", StringComparison.Ordinal),
        "the source bootstrap retains transport, size, or source-authenticity checks when their policy steps are disabled");
    var sourceBuild = ReadSource("source-package", "Install-OpticonFromSource.ps1");
    Assert(!sourceBuild.Contains("[ValidateSet('Production','OwnerManaged')]", StringComparison.Ordinal)
           && !sourceBuild.Contains("[ValidateSet('win-x64', 'win-arm64')]", StringComparison.Ordinal)
           && sourceBuild.Contains("Test-ClientValidationEnabled 'ProtectedPaths'", StringComparison.Ordinal)
           && sourceBuild.Contains("Test-ClientValidationEnabled 'SourceBuildProvenance'", StringComparison.Ordinal),
        "the local source build performs invitation, path, or provenance validation before consulting the client policy");
    var installerServices = ReadSource("src", "Taildesk.Setup", "InstallerServices.cs");
    var unconfirmedCommitStart = installerServices.IndexOf(
        "private async Task CommitWithoutEnrollmentConfirmationAsync", StringComparison.Ordinal);
    var unconfirmedCommitEnd = installerServices.IndexOf(
        "private static bool FixedAsciiEquals", unconfirmedCommitStart, StringComparison.Ordinal);
    var unconfirmedCommit = unconfirmedCommitStart >= 0 && unconfirmedCommitEnd > unconfirmedCommitStart
        ? installerServices[unconfirmedCommitStart..unconfirmedCommitEnd]
        : string.Empty;
    Assert(installerServices.Contains("VerifyPayloadDirectoryCopyIfEnabledAsync", StringComparison.Ordinal)
           && installerServices.Contains("if (!ValidationEnabled(ClientInstallValidationStep.MachineState)) return;", StringComparison.Ordinal)
           && unconfirmedCommit.Contains("ValidationEnabled(ClientInstallValidationStep.ComponentPostconditions)", StringComparison.Ordinal)
           && unconfirmedCommit.Contains("await RequireExactAgentTaskAsync", StringComparison.Ordinal),
        "Setup retains payload, machine-state, or postcondition validation in the disable-all completion path");
...
static void TestReleaseDistributionDesign()
{
    ...
    var deviceClient = Read("src", "Taildesk.Admin", "OpticonDeviceReleaseClient.cs");
    ...
    Assert(sourceOnlyBuilder.Contains("-SourceOnly", StringComparison.Ordinal)
           ...
           && client.Contains("/opticon/artifacts/v1/update-manifest.json", StringComparison.Ordinal)
           && sourceClient.Contains("/opticon/artifacts/v1/update-manifest.json", StringComparison.Ordinal)
           && deviceClient.Contains("/opticon/artifacts/v1/manifest.json", StringComparison.Ordinal)
           && publisher.Contains("/opticon/v1/releases/update-manifest", StringComparison.Ordinal)
           && publisher.Contains("/opticon/artifacts/v1/update-manifest.json", StringComparison.Ordinal)
           && builder.Contains("update-manifest.json", StringComparison.Ordinal)
           && gateway.Contains("updateReleaseAdminPath", StringComparison.Ordinal)
           && gateway.Contains("update-manifest.json", StringComparison.Ordinal)
    ...
    Assert(publisher.Contains("--checksum-algorithm", StringComparison.Ordinal)
           ...
           && publisher.Contains("Add-Type -AssemblyName System.Net.Http", StringComparison.Ordinal)
            ...
            && publisher.Contains(".opticon-release-publisher.lock", StringComparison.Ordinal)
            && publisher.Contains("Enter-OpticonReleasePublisherLock", StringComparison.Ordinal)
            && publisher.LastIndexOf("$releasePublisherLock.Dispose()", StringComparison.Ordinal)
               > publisher.LastIndexOf("FullStreamVerified", StringComparison.Ordinal)
            && builder.Contains("Move-FileReplacingWithRetry -Source $sourceTemporary -Destination $sourceDestination", StringComparison.Ordinal)
            && builder.Contains("another process kept its destination open", StringComparison.Ordinal)
    ...
    Assert(hostedBootstrap.Contains("SimpleDeviceInstaller.RunAsync", StringComparison.Ordinal)
           ...
           && (hostedBootstrap.Contains("ProductSigning.VerifyAuthenticodeAsync", StringComparison.Ordinal)
               || sourceBootstrap.Contains("ProductSigning.VerifyAuthenticodeAsync", StringComparison.Ordinal))
         ...
         "device installer handoff, archive pinning, or redacted persistent Setup diagnostics regressed");
    ...
    Assert(sourceBootstrap.Contains("clearEnvironment: true", StringComparison.Ordinal)
           ...
           && sourceInstaller.Contains("$TargetRuntime -notin @('win-x64', 'win-arm64')", StringComparison.Ordinal)
            ...
            && sourceInstaller.Contains("$SigningProfile -notin @('Production', 'OwnerManaged')", StringComparison.Ordinal)
    ...
    Assert(!builder.Contains("reuseCommandCenterPublish", StringComparison.Ordinal)
           ...
           && builder.Contains("USERPROFILE = $cliHome; HOME = $cliHome", StringComparison.Ordinal)
            ...
            && builder.Contains("Payload\\Uninstall", StringComparison.Ordinal)
            && builder.Contains("Uninstall-Opticon.exe", StringComparison.Ordinal)
...
static void TestReleaseDeploymentSurface()
{
    DirectoryInfo? root = null;
    foreach (var start in new[] { Directory.GetCurrentDirectory(), AppContext.BaseDirectory })
    {
    ...
    Assert(root is not null, "could not find the Opticon source root for release-deployment checks");
    string Read(params string[] parts) => File.ReadAllText(Path.Combine([root!.FullName, .. parts]));
    var window = Read("src", "Taildesk.Admin", "MainWindow.xaml");
    var windowCode = Read("src", "Taildesk.Admin", "MainWindow.xaml.cs");
    var viewModel = Read("src", "Taildesk.Admin", "MainViewModel.cs");
    var service = Read("src", "Taildesk.Admin", "ReleaseDeploymentService.cs");
    var hostedClient = Read("src", "Taildesk.Admin", "HostedInviteClient.cs");
    var invitationService = Read("src", "Taildesk.Admin", "InviteBundleService.cs");
    var config = Read("src", "Taildesk.Admin", "AdminConfig.cs");
    var gateway = Read("fly-headscale", "gateway", "main.go");
    var ensureRelease = Read("scripts", "Ensure-OpticonTargetRelease.ps1");
    var bundlePublisher = Read("fly-headscale", "scripts", "Publish-OpticonBundles.ps1");
    var sourcePublisher = Read("fly-headscale", "scripts", "Publish-OpticonSourceRelease.ps1");
    var sourceBuilder = Read("fly-headscale", "scripts", "Build-OpticonSourceRelease.ps1");
    var bundleBuilder = Read("fly-headscale", "scripts", "Build-OpticonBundles.ps1");
    var buildScript = Read("build.ps1");

    Assert(window.Contains("Invite release deployment", StringComparison.Ordinal)
           && window.Contains("Disable ALL client-side installation validation", StringComparison.Ordinal)
           && window.Contains("ClientValidationOptions", StringComparison.Ordinal)
           && window.Contains("Currently deployed", StringComparison.Ordinal)
           && window.Contains("DeployedReleaseVersion", StringComparison.Ordinal)
           && window.Contains("DeployedReleaseArtifacts", StringComparison.Ordinal)
    ...
    var deploymentIndex = windowCode.IndexOf("DeployReleaseAsync(plan", StringComparison.Ordinal);
    ...
    Assert(service.Contains("Ensure-OpticonTargetRelease.ps1", StringComparison.Ordinal)
           ...
           && service.Contains("version.FileMinorPart >= 1", StringComparison.Ordinal)
            ...
            && service.Contains("install and update artifacts…", StringComparison.Ordinal)
            ...
            && service.Contains("File.ReadAllText(baseIgnore)", StringComparison.Ordinal)
            && !service.Contains("sourceLaunchers", StringComparison.Ordinal)
           ...
           && ensureRelease.Contains("Publish-OpticonBundles.ps1", StringComparison.Ordinal)
           && ensureRelease.Contains("Publish-OpticonSourceRelease.ps1", StringComparison.Ordinal)
           && ensureRelease.Contains("Test-CompleteUpdateRelease", StringComparison.Ordinal)
           && ensureRelease.Contains("/opticon/artifacts/v1/update-manifest.json", StringComparison.Ordinal)
           && ensureRelease.Contains("-CheckOnly", StringComparison.Ordinal)
           && ensureRelease.Contains("-StageOnly", StringComparison.Ordinal)
           && ensureRelease.Contains("SkipManifestPublish", StringComparison.Ordinal)
           && ensureRelease.Contains("SkipBuild", StringComparison.Ordinal)
    ...
    Assert(buildScript.Contains(powerShell71Guard, StringComparison.Ordinal)
           && ensureRelease.Contains(powerShell71Guard, StringComparison.Ordinal)
           && sourcePublisher.Contains(powerShell71Guard, StringComparison.Ordinal)
           ...
           && ensureRelease.Contains("Publish both locally verified release manifests", StringComparison.Ordinal)
    ...
    Assert(gateway.Contains("releasePreflightPath", StringComparison.Ordinal)
           ...
           && gateway.Contains("releaseProtocolVersion    = 5", StringComparison.Ordinal)
...
static void TestOpenSshRecoveryDesign()
{
    ...
    Assert(buildScript.Contains("The Opticon solution build failed", StringComparison.Ordinal)
           ...
           && buildScript.Contains("The clean $ProjectName publish must contain only", StringComparison.Ordinal)
           && buildScript.Contains("if (-not $Incremental) { $solutionBuildArguments += '-t:Rebuild' }", StringComparison.Ordinal)
           && buildScript.Contains("Incremental component reuse is restricted to local OwnerManaged builds", StringComparison.Ordinal)
           && buildScript.Contains("Reusing unchanged $($declaration[0]) publish output", StringComparison.Ordinal)
    ...
    Assert(buildScript.Contains("SkipTargetReleaseDeployment", StringComparison.Ordinal)
           ...
           && targetReleaseCheck.Contains("Test-CompleteUpdateRelease", StringComparison.Ordinal)
           ...
           && targetReleaseCheck.Contains("schemaVersion -ne 1", StringComparison.Ordinal)
           && targetReleaseCheck.Contains("schemaVersion -ne 2", StringComparison.Ordinal)
           && targetReleaseCheck.Contains("OpticonBundle", StringComparison.Ordinal)
           && targetReleaseCheck.Contains("OpticonBootstrap", StringComparison.Ordinal)
